<?php
/**
 * Plugin Name: AnyLLM Connector for OpenAI-Compatible APIs
 * Plugin URI: https://github.com/zjcboy/anyllm-connector-for-openai-compatible-apis
 ...
 * Text Domain: anyllm-connector-for-openai-compatible-apis
 *
...
{
    add_options_page(
        __('OpenAI Compatible AI Settings', 'anyllm-connector-for-openai-compatible-apis'),
        __('AI OpenAI Compatible', 'anyllm-connector-for-openai-compatible-apis'),
        'manage_options',
...
function render_settings_page(): void
{
    ...
    ?>
    <div class="wrap">
        <h1><?php esc_html_e('OpenAI Compatible AI Settings', 'anyllm-connector-for-openai-compatible-apis'); ?></h1>
        <?php if (isset($_GET['cache_cleared']) && $_GET['cache_cleared'] === '1'): ?>
            <div class="notice notice-success is-dismissible" style="margin: 15px 0 0;">
                <p><?php esc_html_e('Models list refreshed and transient cache cleared successfully!', 'anyllm-connector-for-openai-compatible-apis'); ?></p>
            </div>
        <?php endif; ?>
        <form action="options.php" method="post">
            ...
            <table class="form-table" role="presentation">
                <tr valign="top">
                    <th scope="row"><label
                            for="openai_compatible_api_url"><?php esc_html_e('API Base URL', 'anyllm-connector-for-openai-compatible-apis'); ?></label>
                    </th>
                    <td>
                        <input type="text" id="openai_compatible_api_url" name="openai_compatible_api_url"
                            ...
                            class="regular-text" />
                        <p class="description">
                            <?php esc_html_e('The base URL of the OpenAI-compatible API (e.g. https://api.openai.com/v1 or https://api.deepseek.com/v1).', 'anyllm-connector-for-openai-compatible-apis'); ?>
                        </p>
                    </td>
                </tr>
                <tr valign="top">
                    <th scope="row"><label
                            for="openai_compatible_api_key"><?php esc_html_e('API Key', 'anyllm-connector-for-openai-compatible-apis'); ?></label>
                    </th>
                    <td>
                        ...
                        <p class="description">
                            <?php esc_html_e('Your API authentication key.', 'anyllm-connector-for-openai-compatible-apis'); ?>
                            <?php if (defined('OPENAI_COMPATIBLE_API_KEY')): ?>
                                <br />
                                <span
                                    style="color: green; font-weight: bold;"><?php esc_html_e('(Already defined via constant in wp-config.php)', 'anyllm-connector-for-openai-compatible-apis'); ?></span>
                            <?php elseif (get_option('connectors_ai_openai_compatible_api_key')): ?>
                                <br />
                                <span
                                    style="color: green; font-weight: bold;"><?php esc_html_e('(Already configured in Jetpack/WordPress Connectors settings)', 'anyllm-connector-for-openai-compatible-apis'); ?></span>
                            <?php endif; ?>
                        </p>
                    </td>
                </tr>
                <tr valign="top">
                    <th scope="row"><label
                            for="openai_compatible_models"><?php esc_html_e('Custom Models', 'anyllm-connector-for-openai-compatible-apis'); ?></label>
                    </th>
                    <td>
                        <input type="text" id="openai_compatible_models" name="openai_compatible_models"
                            ...
                            class="large-text" />
                        <p class="description">
                            <?php esc_html_e('Comma-separated list of model IDs to make available (e.g., gpt-4o,gpt-4o-mini,deepseek-chat). If empty, the plugin will try to fetch models dynamically from the /models endpoint.', 'anyllm-connector-for-openai-compatible-apis'); ?>
                        </p>
                    </td>
                </tr>
            </table>
        ...
        if (!empty($apiKey)):
            ...
            <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 15px;">
                <h2 style="margin: 0;"><?php esc_html_e('Available Models', 'anyllm-connector-for-openai-compatible-apis'); ?></h2>
                <a href="<?php echo esc_url(wp_nonce_url(admin_url('options-general.php?page=openai-compatible-settings&refresh_models=1'), 'refresh_openai_compatible_models')); ?>" class="button button-secondary">
                    <?php esc_html_e('Refresh Models List', 'anyllm-connector-for-openai-compatible-apis'); ?>
                </a>
            </div>
            <?php if (empty($models)): ?>
                <div class="notice notice-warning inline" style="margin: 0 0 15px;"><p><?php esc_html_e('No models found, or unable to fetch models list from the configured endpoint.', 'anyllm-connector-for-openai-compatible-apis'); ?></p></div>
            <?php else: ?>
                <table class="wp-list-table widefat fixed striped table-view-list" style="margin-top: 15px;">
                    <thead>
                        <tr>
                            <th scope="col" style="width: 30%; font-weight: bold;"><?php esc_html_e('Model Name / ID', 'anyllm-connector-for-openai-compatible-apis'); ?></th>
                            <th scope="col" style="width: 15%; font-weight: bold;"><?php esc_html_e('Context Length', 'anyllm-connector-for-openai-compatible-apis'); ?></th>
                            <th scope="col" style="width: 25%; font-weight: bold;"><?php esc_html_e('Pricing (per 1M tokens)', 'anyllm-connector-for-openai-compatible-apis'); ?></th>
                            <th scope="col" style="width: 30%; font-weight: bold;"><?php esc_html_e('Description', 'anyllm-connector-for-openai-compatible-apis'); ?></th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php foreach ($models as $model):
                            ...
                            $contextLength = isset($model['context_length']) ? number_format((int)$model['context_length']) . ' tokens' : __('N/A', 'anyllm-connector-for-openai-compatible-apis');
                            $description = $model['description'] ?? __('No description provided.', 'anyllm-connector-for-openai-compatible-apis');
            ...
            ?>
            <hr style="margin: 30px 0 20px;" />
            <h2><?php esc_html_e('Available Models', 'anyllm-connector-for-openai-compatible-apis'); ?></h2>
            <p class="description"><?php esc_html_e('Please configure and save your API Key to fetch and display the list of available models.', 'anyllm-connector-for-openai-compatible-apis'); ?></p>
        <?php
        endif;
        ?>
    </div>
    ...
    if (isset($model['pricing']) && is_array($model['pricing'])) {
        ...
        if (null !== $prompt && null !== $completion) {
            if ($prompt == 0 && $completion == 0) {
                return __('Free', 'anyllm-connector-for-openai-compatible-apis');
            }
            ...
            return sprintf(
                __('In: $%s / Out: $%s', 'anyllm-connector-for-openai-compatible-apis'),
                number_format($promptPerMil, 2),
                number_format($completionPerMil, 2)
            );
        }
    }
    return __('N/A', 'anyllm-connector-for-openai-compatible-apis');
}
